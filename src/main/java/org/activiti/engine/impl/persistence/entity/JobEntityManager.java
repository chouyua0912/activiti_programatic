...
public class JobEntityManager extends AbstractManager {
    ...
    public void send(MessageEntity message) {
        ...
        message.insert();
        if (processEngineConfiguration.isAsyncExecutorEnabled()) {  // 等Ｓｐｒｉｎｇ把事务提交了再执行
            hintAsyncExecutor(message);         // 给Hint
        } else {
    ...
    protected void hintAsyncExecutor(JobEntity job) {               // Spring把事务提交了之后会回调　执行任务
        AsyncExecutor asyncExecutor = Context.getProcessEngineConfiguration().getAsyncExecutor();
        ...
        Context.getCommandContext()
                ...
                .addTransactionListener(TransactionState.COMMITTED, transactionListener);       // 事务提交了再提示任务给AsyncJobExecutor
    }
